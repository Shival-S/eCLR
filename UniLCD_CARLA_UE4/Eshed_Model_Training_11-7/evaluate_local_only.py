...
Evaluates ONLY the local model without the routing policy.
...
def evaluate_local_only():
    """
    Evaluate local model across 150 episodes (5 routes × 30 episodes).
    ...
    # Update config for local-only evaluation
    config['env_mode'] = 'eval'  # Enable video recording
    config['rollout_video'] = '/home/shival/UniLCD/Eshed_Model_Training_11-7/rollout_video_local_only.mp4'
    config['minimap_video'] = '/home/shival/UniLCD/Eshed_Model_Training_11-7/minimap_video_local_only.mp4'
    config['task_metrics_path'] = '/home/shival/UniLCD/Eshed_Model_Training_11-7/task_metrics_local_only.csv'
    config['steps_per_episode'] = 1500

    # Use existing pre-trained models
    config['cloud_model_checkpoint'] = '/home/shival/UniLCD/cloud_model.pth'  # Still needed for env initialization
    ...
    print("=" * 70)
    print("LOCAL-ONLY MODEL EVALUATION")
    print("=" * 70)
    ...
    print("(Local model will be used for ALL decisions)")
    print()
    ...
    try:
        for episode in range(n_episodes):
            ...
            while not (done or truncated):
                # ALWAYS use local model (action=0)
