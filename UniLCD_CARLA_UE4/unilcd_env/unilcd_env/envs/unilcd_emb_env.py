...
# InternVL cloud model (optional - loaded if use_internvl=True in config)
try:
    from .il_models.cloud_model_internvl import InternVLCloudModel
...
class UniLCDEmbEnv(gym.Env):
    ...
    def __init__(self, **kwargs):
        ...
        #
        # Check if InternVL cloud model should be used
        self.use_internvl = kwargs.get('use_internvl', False)

        if self.use_internvl:
            if not INTERNVL_AVAILABLE:
                raise ImportError("InternVL model requested but cloud_model_internvl.py not found. "
                                  "Please ensure the InternVL model files are in the il_models directory.")
            print("Loading InternVL cloud model...")
            self.cloud_model = InternVLCloudModel(
                freeze_vision=True,
                use_lora=False,
                pooling="mean"
            )
            self.cloud_model.load_state_dict(torch.load(kwargs['cloud_model_checkpoint'], weights_only=False))
            # InternVL-specific transforms (448x448 input)
            from PIL import Image
            self.internvl_transform = transforms.Compose([
                transforms.ToPILImage(),
                transforms.Resize((448, 448)),
                transforms.ToTensor(),
                transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
            ])
            print("InternVL cloud model loaded successfully")
        else:
        ...
        self.local_model.load_state_dict(torch.load(kwargs['local_model_checkpoint'], weights_only=False))
    ...
    def compute_cloud_policy(self, pedestrian_detected, image, location):
        ...
        if self.use_internvl:
            # InternVL preprocessing: resize to 448x448
            rgb_image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            img_tensor = self.internvl_transform(rgb_image).unsqueeze(0).to(self.device)
        else:
            # Original RegNetY preprocessing
            img_tensor = self.normalizeTransform(self.tensorTransform(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))).unsqueeze(0)

        location_tensor = torch.tensor(location, dtype=torch.float32).unsqueeze(0).to(self.device)
