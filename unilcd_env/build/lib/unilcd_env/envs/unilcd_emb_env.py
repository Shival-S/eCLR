...
class UniLCDEmbEnv(gym.Env):
    ...
    def action_space(self):
        # Continuous action space for SAC: 0.0 = local, 1.0 = cloud
        # Threshold at 0.5 to decide which model to use
    ...
    def _step(self, action):
        ...
        # Extract continuous action value (SAC outputs array)
        action_value = action[0] if hasattr(action, '__len__') else action

        # Preferred inference from local model (action < 0.5)
        if action_value < 0.5:
            mode = "Local"
        ...
        # Preferred inference from cloud model (action >= 0.5)
        else:
            mode = "Cloud"
            start_time = time.time_ns()
